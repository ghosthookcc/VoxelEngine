...
struct blist ProcessPhysics(struct blist* bodylist)
{
  ...
  for(unsigned int i = 0; i < bodylist->size; i++)
  {
    ...
    dvec3 deltavel = multiplyDVec3ByDouble(bodylist->planets[i].velocity, configurations.timestep);
    dvec3 deltaaccel1 = multiplyDVec3ByDouble(accel1, powf(configurations.timestep, 2.0));
    ...
    dvec3 accel12 = multiplyDVec3ByDouble(divideDVec3ByDouble(addDVec3ByDVec3(accel1, accel2), 2.0), configurations.timestep);
